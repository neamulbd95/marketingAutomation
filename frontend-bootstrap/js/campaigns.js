/* ══════════════════════════════════════════
   campaigns.js — Campaign list tab
   ══════════════════════════════════════════ */
...
function initCampaignTab() {
  ...
  $('#camp-search').on('input', debounce(function () {
    ...
    $('#camp-search-clear').toggleClass('d-none', !campState.search);
    campState.page = 1; loadCampaigns();
  }, 350));

  $('#camp-search-clear').on('click', function () {
    $('#camp-search').val(''); campState.search = ''; $(this).addClass('d-none');
    campState.page = 1; loadCampaigns();
  });
  ...
  $('#camp-filter-offcanvas')
    .on('show.bs.offcanvas', () => $('#camp-filter-btn').addClass('active'))
    .on('hidden.bs.offcanvas', () => $('#camp-filter-btn').removeClass('active'));
}
...
function loadCampaigns() {
  $('#camp-tbody').html(`<tr><td colspan="7" class="text-center py-5"><div class="spinner-border spinner-brand"></div></td></tr>`);
  ...
  const html = data.map(c => {
    ...
    return `<tr class="${checked?'selected':''} clickable-row">
      <td class="td-check" onclick="event.stopPropagation()">
        <input type="checkbox" class="form-check-input" data-id="${c.id}" ${checked?'checked':''} onchange="toggleCampaignRow(this,'${c.id}')" />
      </td>
      <td onclick="location.href='campaign-detail.html?id=${c.id}'">
        ...
        ${folderName?`<div style="font-size:11px;color:#94a3b8;margin-top:2px"><i class="fa-regular fa-folder me-1"></i>${escHtml(folderName)}</div>`:''}
      </td>
...
function updateCampaignBulkActions() {
  ...
  $('#camp-bulk-delete-btn').toggleClass('d-none', n===0);
  $('#camp-move-btn').toggleClass('d-none', n===0);
...
function showCampaignModal() {
  ...
  setTimeout(() => $('#camp-name').trigger('focus'), 150);
}
function saveCampaign() {
  ...
  req.done(r => {
    if (r.success) { showToast(r.message,'success'); hideModal('campaignModal'); loadCampaigns(); loadCampaignFolderCounts(); updateSidebarCounts(); }
  }).fail(() => showToast('Failed','error'));
}
...
function applyCampaignFilters() {
  ...
  $('#camp-filter-badge').text(n).toggleClass('d-none', n===0);
  loadCampaigns();
  hideOffcanvas('camp-filter-offcanvas');
}
function clearCampaignFilters() {
  ...
  $('#camp-filter-badge').addClass('d-none'); campState.page=1; loadCampaigns();
}
