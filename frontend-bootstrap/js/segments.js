...
function initSegmentTab() {
  ...
  $('#seg-search').on('input', debounce(function () {
    ...
    $('#seg-search-clear').toggleClass('d-none', !segState.search);
    segState.page = 1; loadSegments();
  }, 350));

  $('#seg-search-clear').on('click', function () {
    $('#seg-search').val(''); segState.search = ''; $(this).addClass('d-none');
    segState.page = 1; loadSegments();
  });
  ...
  $('#seg-filter-offcanvas')
    .on('show.bs.offcanvas', () => $('#seg-filter-btn').addClass('active'))
    .on('hidden.bs.offcanvas', () => $('#seg-filter-btn').removeClass('active'));
}
...
function loadSegments() {
  $('#seg-tbody').html(`<tr><td colspan="7" class="text-center py-5"><div class="spinner-border spinner-brand"></div></td></tr>`);
  ...
  const html = data.map(s => {
    ...
    return `<tr class="${checked?'selected':''} clickable-row">
      <td class="td-check" onclick="event.stopPropagation()">
        <input type="checkbox" class="form-check-input" data-id="${s.id}" ${checked?'checked':''} onchange="toggleSegmentRow(this,'${s.id}')" />
      </td>
...
function updateSegmentBulkActions() {
  ...
  $('#seg-bulk-delete-btn').toggleClass('d-none', n===0);
  $('#seg-move-btn').toggleClass('d-none', n===0);
...
function showSegmentModal() {
  ...
  setTimeout(() => $('#seg-name').trigger('focus'), 150);
}
function saveSegment() {
  ...
  req.done(r => {
    if (r.success) { showToast(r.message,'success'); hideModal('segmentModal'); loadSegments(); loadSegmentFolderCounts(); updateSidebarCounts(); }
  }).fail(() => showToast('Failed','error'));
}
...
function applySegmentFilters() {
  ...
  $('#seg-filter-badge').text(n).toggleClass('d-none', n===0);
  loadSegments();
  hideOffcanvas('seg-filter-offcanvas');
}
function clearSegmentFilters() {
  ...
  $('#seg-filter-badge').addClass('d-none'); segState.page=1; loadSegments();
}
