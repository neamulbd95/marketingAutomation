/* ══════════════════════════════════════════
   email-template.js — Email Template list tab
   ══════════════════════════════════════════ */
...
function initEtTab() {
  ...
  $('#et-search').on('input', debounce(function () {
    ...
    $('#et-search-clear').toggleClass('hidden', !etState.search);
    etState.page = 1; loadEmailTemplates();
  }, 350));

  $('#et-search-clear').on('click', function () {
    $('#et-search').val(''); etState.search = ''; $(this).addClass('hidden');
    etState.page = 1; loadEmailTemplates();
  });
...
function loadEmailTemplates() {
  $('#et-tbody').html(`<tr><td colspan="7" class="table-empty"><div class="spinner"></div></td></tr>`);
  const params = {
  ...
  apiGet('/email-templates', params).done(r => {
    ...
    $('#et-tbody').html(`<tr><td colspan="7"><div class="no-data-msg"><i class="fa-solid fa-triangle-exclamation"></i><p>Failed to load. Is the API running?</p></div></td></tr>`);
  });
}
  ...
  const html = data.map(t => {
    ...
    return `<tr class="${checked?'selected':''} clickable-row">
      <td class="td-check" onclick="event.stopPropagation()">
        <input type="checkbox" data-id="${t.id}" ${checked?'checked':''} onchange="toggleEtRow(this,'${t.id}')" />
      </td>
      ...
      <td class="td-action" onclick="event.stopPropagation()">
        <button class="btn-row-preview" onclick="previewEt('${t.id}')" title="Preview template">
...
function updateEtBulkActions() {
  ...
  $('#et-bulk-delete-btn').toggleClass('hidden', n===0);
...
function showEtModal() {
  ...
  $('#et-modal-overlay').removeClass('hidden');
  setTimeout(() => $('#et-name').focus(), 80);
}
function closeEtModal() { $('#et-modal-overlay').addClass('hidden'); }
function saveEt() {
  ...
  req.done(r => {
    if (r.success) { showToast(r.message,'success'); closeEtModal(); loadEmailTemplates(); }
  }).fail(() => showToast('Failed','error'));
}
...
function applyEtFilters() {
  ...
  $('#et-filter-badge').text(n).toggleClass('hidden', n===0);
  loadEmailTemplates();
}
function clearEtFilters() {
  ...
  $('#et-filter-badge').addClass('hidden'); etState.page = 1; loadEmailTemplates();
}
...
function previewEt(id) {
  ...
  $('#et-preview-modal-overlay').removeClass('hidden');
  apiGet('/email-templates/' + id).done(r => {
    if (!r.success) { showToast('Failed to load preview', 'error'); closeEtPreviewModal(); return; }
    const t = r.data;
  ...
  }).fail(() => { showToast('Failed to load preview', 'error'); closeEtPreviewModal(); });
}

function closeEtPreviewModal() {
  $('#et-preview-modal-overlay').addClass('hidden');
