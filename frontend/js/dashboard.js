...
function initDashboard() {
  ...
  $('.chart-legend-btn').on('click', function () {
    $('.chart-legend-btn').removeClass('active');
...
function loadDashboardStats() {
  $.when(
    ...
    $('#stat-campaigns').text(formatNumber(total));
    ...
    $legend.append(`
      <div class="donut-legend-item">
        <span class="donut-legend-label">
          <span class="donut-legend-dot" style="background:${colors[i]}"></span>${s}
        </span>
...
function renderCalendar() {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    if (Math.random() > 0.65) {
      ...
      sampleEvents[i] = Array.from({length: count}, (_, j) => ({
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const eventsHtml = events.slice(0, 2).map(e =>
      `<span class="day-event-dot type-${e.type}" title="${e.name}">${e.name}</span>`
    ).join('');
...
function buildActivityFeed() {
  const activities = [
    { icon: 'fa-bullhorn', color: 'bg-blue-100 text-blue-600', text: '<strong>Summer Sale 2025</strong> campaign was activated', time: '2 minutes ago' },
    { icon: 'fa-users', color: 'bg-purple-100 text-purple-600', text: 'New segment <strong>Premium Users Q2</strong> created', time: '18 minutes ago' },
    { icon: 'fa-envelope', color: 'bg-amber-100 text-amber-600', text: '<strong>Welcome Series v2</strong> template updated', time: '45 minutes ago' },
    { icon: 'fa-folder-plus', color: 'bg-green-100 text-green-600', text: 'Folder <strong>Product Launch</strong> created', time: '2 hours ago' },
    { icon: 'fa-pause-circle', color: 'bg-orange-100 text-orange-600', text: '<strong>Flash Sale Alert</strong> campaign paused', time: '3 hours ago' },
    { icon: 'fa-chart-bar', color: 'bg-indigo-100 text-indigo-600', text: 'Weekly performance report generated', time: '5 hours ago' },
    { icon: 'fa-trash', color: 'bg-red-100 text-red-600', text: '<strong>Old Newsletter Mar</strong> template archived', time: '1 day ago' },
    { icon: 'fa-check-circle', color: 'bg-teal-100 text-teal-600', text: '<strong>Q1 Retention Drive</strong> completed successfully', time: '1 day ago' },
  ];
  ...
  const html = activities.map(a => `
    <div class="activity-item">
      ...
      <div class="activity-body">
        ...
        <div class="activity-time"><i class="fa-regular fa-clock mr-1"></i>${a.time}</div>
      </div>
    </div>
  `).join('');
